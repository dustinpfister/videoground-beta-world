/*    video05-03-sawfrom-alt-offset2 - for audio-generator-waveform project
          * yet another one this time having the offset move for both saw tooth sample sets
          * Back and forth a few times to hear that effect
 */
//-------- ----------
// SCRIPTS
//-------- ----------
...
VIDEO.init = function(sm, scene, camera){
    ...
    sq.objects[0] = {
        ...
        for_frame: (fs, frame, max_frame, a_sound2, opt, a_object, sq) => {
            const pa_1 = 2.66 * opt.secs;
            const pa_2 = 5.33 * opt.secs;
            const a_off = Math.sin(Math.PI * ( a_object * 8 % 1) );
            saw_samps(fs.array_wave, 100, a_sound2, pa_1, false, 0.75 * a_off );
            saw_samps(fs.array_wave, 100, a_sound2, pa_2, true,  0.75 * a_off );
            return fs;
        },
