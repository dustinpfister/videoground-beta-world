/*    video03-03-import-quack - for audio-generator-waveform project
          * I would like to read wav files directly
          * I am also going to work out some new methods for this kind of thing
          * oh yeah, and how about quacking over meowing in terms of the source data
 */
//-------- ----------
// SCRIPTS
//-------- ----------
...
VIDEO.init = function(sm, scene, camera){
    ...
    sq.objects[1] = {
        alpha: 6 / total_secs,
        for_frame: (fs, frame, max_frame, a_sound2, opt, a_object, sq) => {
            const fpi = 5 - Math.round(4 * a_object);
            fs.array_wave = scene.userData.array_wave = ST.get_waveform_array(fs.array_import, fpi, frame );
            return fs;
        },
        for_sampset: function(samp, i, a_sound, opt, a_object, sq){
    ...
    sq.objects[2] = {
        alpha: 9 / total_secs,
        for_frame: (fs, frame, max_frame, a_sound2, opt, a_object, sq) => {
            const fpi = 1 + Math.round(4 * a_object);
            fs.array_wave = scene.userData.array_wave = ST.get_waveform_array(fs.array_import, fpi, frame );
            return fs;
        },
        for_sampset: function(samp, i, a_sound, opt, a_object, sq){
    ...
    sq.objects[3] = {
        ...
        for_frame: (fs, frame, max_frame, a_sound2, opt, a_object, sq) => {
            const fpi = 5
            fs.array_wave = scene.userData.array_wave = ST.get_waveform_array(fs.array_import, fpi, frame );
            fs.freq = 0.25 + 2.75 * Math.sin(Math.PI * a_object);
            return fs;
        },
        for_sampset: function(samp, i, a_sound, opt, a_object, sq){
            return samp;  
