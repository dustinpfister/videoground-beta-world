...
VIDEO.init = function(sm, scene, camera){
    ...
    const curve_amp = sud.curve_amp = ST.get_curve_path([ 
        [0.00, 0.50, 0.80, 0.50, 0.50, 0.60, 0.80]
    ]);
    const curve_param = sud.curve_param = ST.get_curve_path([ 
        [0.00, 0.05, 0.00, 0.05, 0.00, 0.00, 0.00],
        [0.10, 0.45, 0.50, 0.45, 0.50, 0.00, 0.00],
        [0.50, 0.70, 0.80, 0.70, 0.80, 0.00, 0.00],
    ]);
    // main sound object
...
VIDEO.render = function(sm, canvas, ctx, scene, camera, renderer){
    ...
    DSD.draw_curve( ctx, sud.curve_param, alpha, 'param ( saw effect ) ', sud.opt_curve_param );
